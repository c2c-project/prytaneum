...
function loadRoot() {
    let { loadedEnvFiles } = loadEnvConfig(path.join(__dirname, '../'));
    processEnv(loadedEnvFiles);
}

function loadDB() {
    let { loadedEnvFiles } = loadEnvConfig(path.join(__dirname, '../app/db'));
    processEnv(loadedEnvFiles);
}

function loadServer() {
    let { loadedEnvFiles } = loadEnvConfig(path.join(__dirname, '../app/server'));
    processEnv(loadedEnvFiles);
}

function loadClient() {
    let { loadedEnvFiles } = loadEnvConfig(path.join(__dirname, '../app/client'));
    processEnv(loadedEnvFiles);
}
